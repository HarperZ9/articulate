...
content, so an edited, removed or reordered entry breaks the chain, and a broken
log is never extended. The chain design follows the draft log proposed in pull
request #3 (GENESIS, entry_hash, refusal to extend a broken log).

`init` writes a `.gitignore` containing `*` in the log folder, so `git add -A`
cannot sweep a private log into a repository; `track=True` leaves it out for a
writer who wants the log in git. Nothing records until the writer runs a
command. No network call, no subprocess.
...
def paths(doc):
    """The log, the private salts file, the private diff cache and snapshots."""
    base = os.path.join(folder(doc), os.path.basename(doc))
    return {"log": base + ".jsonl", "salts": base + ".salts.json",
            "last": base + ".last.txt", "objects": os.path.join(folder(doc), "objects")}
...
def append(doc, kind, fields, now=None):
    ...
    entries, problems = load(doc)
    problems += chain_problems(entries)
    if problems:
        raise LogBroken("the process log is broken, so nothing was appended: "
                        + "; ".join(problems[:3]) + ". Run `articulate process continue`.")
...
def init(doc, track=False, opt_in=(), now=None):
    ...
    ignore = os.path.join(folder(doc), ".gitignore")
    if not track and not os.path.exists(ignore):
...
def restart(doc, reason, now=None):
    ...
    with a stated reason and never as a fresh start."""
    p = paths(doc)
    entries, _ = load(doc)
    ...
    return _write(p["log"], [], "continuation",
                  {"continues": anchor, "reason": reason, "opt_in": sorted(opts)}, now)
